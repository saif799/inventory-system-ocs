...
- **The admin picker groups the catalogue by Shoe Model and takes a whole model in one click. That is a bulk-add affordance, not membership.** The colours it adds are copied in as ordinary hand-picked rows, and each one can be removed or reordered on its own afterwards. A **live model rule** — storing "model X" on the Collection and resolving its colours at render time, so new colours join automatically — was on the table when that picker was designed and was **declined**: it is the classification model this ADR already rejected, it takes away per-colour removal and hand ordering, and it needs a schema change to hold. The consequence is that a colour added to a model later joins no Collection; re-opening the Collection and ticking the model again is the intended path.
